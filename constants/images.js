const hammad = require("../assets/images/hammad.jpg");
const profile_pic = require("../assets/images/profile_pic.jpg");
const background1 = require("../assets/images/appbg.png");
const background2 = require("../assets/images/appbg2.png");
const logoBackground = require("../assets/images/logobg.png")
const creditCardBg = require("../assets/images/creditcardbg.jpg")
const logobg = require("../assets/images/quickbusLogoBgRemoved.png")
export default {
    profile_pic,
    hammad,
    background1,
    background2,
    logoBackground,
    creditCardBg,
    logobg
}